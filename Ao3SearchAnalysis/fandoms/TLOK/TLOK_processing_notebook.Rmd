...
rm(seasonsStart, seasonsEnd, dates, plotDatesDensity)
...
* Both Complete Works and Works in Progress for TLOK are longer than works for TDP.
...
* Number of kudos doesn't quite scale up with the number of hits, but unlike in TDP case E rated works also get the most kudos (~220). The difference in distribution might be due to the fact that TLOK fandom on average skews older.
* Works rated M get the most comments (>50) whihc might correlate with them being the longest.
...
Out of the 3 main shipping categories, in absolute numbers "F/F" has most E rated works, and "M/M" has the least. However, in relative amounts "M/M" category has `r round(100*sum(statsCategory$Category == "M/M" & statsCategory$Rating == "Explicit")/sum(statsCategory$Category == "M/M"))`%, "F/F" has `r round(100*sum(statsCategory$Category == "F/F" & statsCategory$Rating == "Explicit")/sum(statsCategory$Category == "F/F"))`%, and "F/M" has `r round(100*sum(statsCategory$Category == "F/M" & statsCategory$Rating == "Explicit")/sum(statsCategory$Category == "F/M"))`% E rated works.
...
Overwhelmingly, "Korra/Asami Sato"/"Korrasami"/"Korra/Asami" is the most popular relationship in TLOK, contributing to popularity of "F/F" category. They are followed by "Korra/Mako (Avatar)", and "Bolin/Opal (Avatar)".
