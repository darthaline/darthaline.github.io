...
for (linkCounter in 1:length(linkList)){
  ...
  while(!readSuccess){
    readSuccess <- TRUE
    tryCatch(urlActive <- url(linkList[linkCounter], "rb"), 
             error = function() {
               readSuccess <- FALSE
               Sys.sleep(60)
             })
  ...
  for (i in 1:length(worksList)) {
    ...
    #filtering warnings
    warningsTemp <- html_text(html_nodes(worksList[i], '.warnings'))
    warningsTemp <- warningsTemp[warningsTemp == ""]
    warningsTemp[warningsTemp == "Choose Not To Use Archive Warnings"] <- "Creator Chose Not To Use Archive Warnings"
    ...
    tempList <- list('Title' = html_text(titleTemp[1]),
                     ...
                     'Warnings' = unique(warningsTemp),
                     'Category' = strsplit(html_text(html_nodes(worksList[i], '.category')), ', ')[[1]],
                     'Completion Status' = unique(html_text(html_nodes(worksList[i], '.iswip'))),
                     'Date' = dateTemp,
                     'Nonfiction' = metaTemp,
                     'Relationships Count' = length(html_nodes(html_nodes(worksList[i], '.tags'), '.relationships')),
                     'Character Count' = length(html_nodes(html_nodes(worksList[i], '.tags'), '.characters')),
                     'Freeform Count' = length(html_nodes(html_nodes(worksList[i], '.tags'), '.freeforms')),
    ...
    rm(titleTemp, authorTemp, fandomTemp, warningsTemp, dateTemp, metaTemp, statsTemp, chapterTemp, tempList)
  }
...
save(worksData, file='worksData.RData')
